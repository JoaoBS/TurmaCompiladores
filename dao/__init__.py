...
def conectar_cloudBD():
    con = psycopg2.connect(
        host= 'dpg-cnser8uct0pc73d6uk60-a.oregon-postgres.render.com',
        database= 'bd_nuvem',
        user= 'bd_nuvem_user',
        password= 'ormnM5ZtDdoqIQfGvlxf6ZI3TNGjSqri'
    )
    return con
...
def verificarUsuarioExistente(email):
    conexao = conectarDB()
    cur = conexao.cursor()
    cur.execute(f"select count(*) from public.\"Usuarios\" where email = '{email}'")
    recset = cur.fetchall()
    conexao.close()
    if recset[0][0] == 1:
        return True
...
def cadastrarusuario(nome, idade, email, senha):
    if not verificarUsuarioExistente(email):
        ...
        cur = conexao.cursor()
        try:
            sql = f"INSERT INTO public.\"Usuarios\" (nome, idade, email, senha) VALUES ('{nome}', '{idade}', '{email}', '{senha}' )"
            cur.execute(sql)
...
def checarlogin(email, senha):
    conexao = conectarDB()
    cur = conexao.cursor()
    cur.execute(f"select count(*) from public.\"Usuarios\" where email = '{email}' and senha ='{senha}'")
    recset = cur.fetchall()
    conexao.close()
    if recset[0][0] == 1:
        return True
...
def registrar_contato(nome, email, comentario, cep):
    ...
    cur = conexao.cursor()
    try:
        sql = (f"INSERT INTO public.\"Contatos\" (nome, email_contato, mensagem, rua, cidade, estado)"
               f" VALUES ('{nome}', '{email}', '{comentario}', '{rua}', '{cidade}', '{estado}')")
        cur.execute(sql)
    except psycopg2.IntegrityError:
        conexao.rollback()
        exito = False
...
def listarUsuarios():
    conexao = conectarDB()
    cur = conexao.cursor()
    cur.execute(f"select nome, idade, email from public.\"Usuarios\"")
    recset = cur.fetchall()
    conexao.close()
