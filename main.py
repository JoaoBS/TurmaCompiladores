...
def inserircontato():
    ...
    if dao.registrar_contato(nome, email, texto, cep):
        return render_template('logado.html', msg='Comentário enviado')
    else:
        #criar pagina de erro de contato
        return render_template('errocontato.html', msg='Não foi possível enviar comentário')


@app.route('/exibirusuarios')
def exibirUsuarios():

    headings = ('Nome', 'Idade', 'E-mail')
    dados = dao.listarUsuarios()

    return render_template('tabelausuarios.html', headings=headings, data=dados)
